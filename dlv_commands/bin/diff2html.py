...
import sys, re, html.entities, getopt, codecs, datetime
...
def add_comment(s, output_file):
    print(('<tr class="diffmisc"><td colspan="4">%s</td></tr>\n'%convert(s)))


def add_filename(f1, f2, output_file):
    print(("<tr><th colspan='2'>%s</th>"%convert(f1, linesize=linesize)))
    print(("<th colspan='2'>%s</th></tr>\n"%convert(f2, linesize=linesize)))


def add_hunk(output_file, show_hunk_infos):
    if show_hunk_infos:
        print('<tr class="diffhunk"><td colspan="2">Offset %d, %d lines modified</td>'%(hunk_off1, hunk_size1))
        print('<td colspan="2">Offset %d, %d lines modified</td></tr>\n'%(hunk_off2, hunk_size2))
    else:
        # &#8942; - vertical ellipsis
        print('<tr class="diffhunk"><td colspan="2">&#8942;</td><td colspan="2">&#8942;</td></tr>')
...
def add_line(s1, s2, output_file):
    ...
    print(('<tr class="diff%s">' % type_name))
    if s1 != None and s1 != "":
        print(('<td class="diffline">%d </td>' % line1))
        print('<td class="diffpresent">')
        print(convert(s1, linesize=linesize, ponct=1))
        print('</td>')
    else:
        s1 = ""
    ...
    if s2 != None and s2 != "":
        print(('<td class="diffline">%d </td>'%line2))
        print('<td class="diffpresent">')
        print(convert(s2, linesize=linesize, ponct=1))
        print('</td>')
    else:
        s2 = ""
        print('<td colspan="2"></td>')
...
def parse_input(input_file, output_file, input_file_name, output_file_name,
    ...
    if not exclude_headers:
        ...
        print(html_hdr.format(title_suffix, encoding, desc, "", modified_date, lang))
    print(table_hdr)
    ...
    print(table_footer)
    if not exclude_headers:
        print(html_footer.format("", dtnow.strftime("%d.%m.%Y")))


def usage():
    print("see code by urself")
"""
    print '''
